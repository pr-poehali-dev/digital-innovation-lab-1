...
export interface POBotConfig {
  ...
  /** Размер хеджа-3 как множитель от основной (X). Открывается ПОСЛЕ Хедж-2, СОВПАДАЕТ с основной */
  ...
  pipSize: number
  /** Авто-определение размера пипса по активу (JPY=0.01, BTC=1.0, остальные=0.0001). Если выкл — используется pipSize */
  pipSizeAuto?: boolean
...
// ===== СТАРЫЙ ГЕНЕРАТОР УДАЛЁН (рефакторинг 2026-05, коммит abb10c6) =====
// generatePOCode был дубликатом generatePOComboCode (~3500 строк). Удалён физически.
// BotBuilder.tsx вызывает только generatePOComboCode. Если comboMode выключен —
// обёртка-алиас ниже кладёт текущую strategy в массив comboStrategies автоматически.
/* DELETED_BLOCK_START
